...
mut re_ppcre_version := ppcre.pcre_compile(version_pat_others, 0)!
re_ppcre_version.exec(version_line1, 0)!
re_ppcre_version.exec(version_line2, 0)!
if _ := re_ppcre_version.exec(version_line3, 0) {
	panic('re_ppcre_version.exec(version_line3, 0)')
}
mut re_ppcre_commit := ppcre.pcre_compile(commit_pat_others, 0)!
re_ppcre_commit.exec(commit_line1, 0)!
if _ := re_ppcre_commit.exec(commit_line2, 0) {
	panic('re_ppcre_commit.exec(commit_line2, 0)')
}
...
for _ in 0 .. repeat_count_version {
	for line in changes {
		re_ppcre_version.matches(line, 0)!
	}
}
...
for _ in 0 .. repeat_count_version {
	for line in changes {
		re_ppcre_version.exec(line, 0) or {
			if err !is ppcre.NoMatch {
				panic(err)
			}
		}
	}
...
for _ in 0 .. repeat_count_commit {
	re_ppcre_commit.exec(commit_line1, 0)!
	re_ppcre_commit.group_index_by_name('type')
	re_ppcre_commit.group_index_by_name('description')
}
b.measure('prantlf.pcre named')

for _ in 0 .. repeat_count_commit {
	re_ppcre_commit.exec(commit_line2, 0) or {
		if err !is ppcre.NoMatch {
			panic(err)
		}
	}
